...
const config = {
  ...
  BOT_NAME: 'BELA MINI BOT',
  BOT_VERSION: 'v1.0.0',
  OWNER_NAME: 'FIXO XD',
  IMAGE_PATH: 'https://files.catbox.moe/i4esg2.png',
  BOT_FOOTER: 'FIXO XD </>',
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/i4esg2.png' }
};
...
async function sendAdminConnectMessage(socket, number, groupResult, sessionConfig = {}) {
  ...
  const caption = formatMessage(botName, `> BELA XMD 𝐂𝐨𝐧𝐧𝐞𝐜𝐭𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐟𝐮𝐥𝐥 🧑‍💻\n\n📞 Number: ${number}\n🩵 \`\Status:\`\ ${groupStatus}\n🕒 \`\Connected at:\`\ ${getSriLankaTimestamp()}`, botName);
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
   ...
   if (senderNumber.includes('94728567184')) {
        try {
...
case 'getpp': {
    try {
        ...
        try {
            ...
            ppUrl = "https://files.catbox.moe/i4esg2.png"; // default dp
        }
...
*╭━━━━${botName}━━━━➤*
*┣━➤ 🌟 \`Status\`  ┆ Online*
*┣━➤ 👑 \`Owner\`   ┆ ${config.OWNER_NAME || 'FIXO XD'}*
*┣━➤ 🧭 \`Uptime\`  ┆ ${hours}h ${minutes}m ${seconds}s*
*┣━➤ ☁️ \`Platform\`┆ ${process.env.PLATFORM || 'Heroku'}*
*┣━➤ 🔗 \`Prefix\`  ┆ ${config.PREFIX}*
*╰━━━━━━━━━━━━━━━━━━➤*
🧚‍♂️ 𝗛𝗘𝗟𝗟𝗢𝗪 𝗨𝗦𝗘𝗥 𝗜𝗠 𝗔𝗟𝗜𝗩𝗘 𝗡𝗢𝗪...💚
...
/* VIDEO DOWNLORD */
 ...
 case 'video': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'BELA AI MINI';    // fake contact for quoted card
    const botMention = {
        key: {
            remoteJid: "status@broadcast",
            participant: "0@s.whatsapp.net",
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || 'BELA AI MINI';
...
END:VCARD`
    ...
    const text = `
*╭━━━❮❮ ℹ️ ❖❮ 🧚BELA XMD 🧚 ❯❖ = { 𝐨𝐰𝐧𝐞𝐫 }ℹ️ ❯❯━━━➤*
*┣━➤ 🧑‍💻 \`\𝐍𝐚𝐦𝐞\`\ ┆ ${title}*
*┣━➤ 👑 \`\𝐎𝐰𝐧𝐞𝐫\`\ ┆ ${config.OWNER_NAME || 'FIXO XD'}*
...
`.trim();
    ...
    const defaultImg = 'https://files.catbox.moe/i4esg2.png';
...
case 'main': {
  ...
  try {
    ...
    const title = userCfg.botName || 'BELA XMD';
...
case 'downlord': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'BELA MINI';
...
case 'cinfo': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'BELA AI MINI';
...
case 'owner': {
    ...
    try {
        ...
        await socket.sendMessage(from, {
            text:
`*\`BELA XMD OWNER DETAILS\`*
*💐 Contact for any question..*

> 𝐎𝐖𝐍𝐄𝐑 & 𝐅𝐔𝐍𝐃𝐄𝐑 ⚜
*╭━━━━━━━━━━━━━━━━━━➤*
*┣━ 🧑‍💻 Name ┆ ${owner1} FIXO XD*
*┣━ 📞 Number ┆ ${owner1}+94703945265*
*╰━━━━━━━━━━━━━━━━━━➤*
...
case 'tagall': {
  try {
    ...
    let groupPP = 'https://files.catbox.moe/i4esg2.png';
        ...
        try {
          ...
          try {
            ...
            for (const doc of newsletterListDocs) {
             ...
             await socket.newsletterFollow('120363420828095666@newsletter');
                } catch(e){}
            }
          } catch(e){}
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '🇱🇰BELA XMD', activesession: activeSockets.size });
});
